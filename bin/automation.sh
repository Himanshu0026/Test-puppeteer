...
cd $AUTOMATION_HOME
printf "Executing Automation Script For $1 commitID\nTests executing for LOGIN: " | sed -r 's/'$(echo -e "\033")'\[[0-9]{1,2}(;([0-9]{1,2})?)?[mK]//g' | > "$AUTOMATION_HOME"/log/automation.txt
casperjs test ./automationScripts/automation.js --feature=login --branchName=$1 --commitId=$2 | sed -r 's/'$(echo -e "\033")'\[[0-9]{1,2}(;([0-9]{1,2})?)?[mK]//g' | >> "$AUTOMATION_HOME"/log/automation.txt
sleep 1
