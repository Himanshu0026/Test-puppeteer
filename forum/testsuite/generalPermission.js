		...
		forumLogin.loginToApp(json['deleteAccount'].uname, json['deleteAccount'].upass, casper, function() {
			casper.wait(5000, function() {
				...
				try {
					...
					this.echo('Enabled View Profile Is Verified', 'INFO');
				}catch(e) {
					test.assertExists('div.text-center.bmessage.alert-info.text-danger');
					this.echo('View Profile Is Disabled From The Back-end', 'ERROR');
				}
			});
		});
	});
					...
					try {
						...
						casper.wait(5000, function() {
							...
							try {
								...
								var successMsg = this.fetchText('font[color="red"]');
								var expectedSuccessMsg = 'Your user group settings have been updated.';
								verifySuccessMsg(successMsg, expectedSuccessMsg, 'UncheckedEditProfile', casper, function() {
									casper.capture(screenShotsDir+ 'success.png');
								});
							}catch(e) {
								test.assertDoesntExist('font[color="red"]');
				...
				try {
					...
					casper.wait(5000, function() {
						try {
							...
							casper.wait(5000, function() {
								try {
									var user = x('//a/strong[text()="hs1234"]/ancestor::li/span/a');
...
//***********************************7th Test Case Verification**********************************
		...
		try {
			...
			try {
				...
				casper.wait(5000, function() {
					...
					try {
						test.assertExists('a[href^="/tool/members/mb/skins?action=install_skin&subaction=skins&skin_id=9&search_skin=&sorted="]');
						this.click('a[href^="/tool/members/mb/skins?action=install_skin&subaction=skins&skin_id=9&search_skin=&sorted="]');
						casper.then(function() {
							this.on('remote.alert', testAlert2);
						});
					}catch(e) {
						test.assertDoesntExist('a[href^="/tool/members/mb/skins?action=install_skin&subaction=skins&skin_id=9&search_skin=&sorted="]');
					}	
					casper.wait(5000, function() {
						this.then(function() {
	...
	casper.then(function() {
		forumLogin.loginToApp(json['deleteAccount'].uname, json['deleteAccount'].upass, casper, function() {
			casper.wait(5000, function() {
				test.assertExists('a.username.usergroup946060');
				this.click('a.username.usergroup946060');
				casper.wait(5000, function() {
