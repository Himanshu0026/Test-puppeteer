...
forumLoginTests.invalidPassword = function() {
	...
	forumLoginMethod.loginToApp(json['InvalidPassowrd'].username, json['InvalidPassowrd'].password, casper, function(err){
		if(!err){
			...
			loginWait.waitElement('form[name="frmLogin"] [role="alert"]', casper , function(err, isExists){
				if(isExists) {
			...
			if(!err) {
				...
				loginWait.waitElement('form[name="frmLogin"] [role="alert"]', casper, function(err,isExists) {
					if(isExists) {
						...
						errorMessage = errorMessage.trim();
						if(errorMessage && errorMessage!= '')
							forumLoginMethod.verifyErrorMsg(errorMessage, 'You must enter your password.', 'blankEmailPassword', casper, function() {});
					}
				});
			};
		});
